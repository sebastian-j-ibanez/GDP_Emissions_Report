...
print('India\'s Emissions R2: ', yr2)

zPredicted = zmodel(x)
zr2 = metrics.r2_score(z, zPredicted)
print('India\'s GDP R2: ', zr2)

myline = np.linspace(1960, 2040, 100)

plt.subplot(2, 1, 1)
plt.plot(x, y, color='#ff595e')
plt.plot(myline, ymodel(myline), color='#ffb703', linestyle='--')
plt.title('India\'s Emissions - Polynomial Predicted Model (3rd Degree)')
...
plt.title('India\'s GDP - Polynomial Predicted Model (3rd Degree)')
plt.legend(['Actual', 'Predicted'])
plt.xlabel('Year')
plt.ylabel('GDP (USD)')
plt.show()